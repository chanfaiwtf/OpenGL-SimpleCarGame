...
void CParticleSystemTransformFeedback::UpdateParticles(float fTimePassed)
{
	...
	glEnable(GL_RASTERIZER_DISCARD);	//disable output
	...
	glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 0, uiParticleBuffer[1-iCurReadBuffer]); //indicate the buffer we want to save the result
...
void CParticleSystemTransformFeedback::RenderParticles()
{
	...
	//ljt
	spRenderParticles.SetUniform("matrices.mModel", &matModel);
